...
import { sendWxMessage } from "../utils/wxGroupBot.js";
  ...
  try {
    ...
    if (pairAge <= MAX_AGE_DAYS && tokenInfo.marketCap >= MIN_MARKET_CAP) {
      ...
      if (tgResponse?.ok === true) {
        ...
        console.log(
          `[${getTimeStamp()}] Successfully sent analysis for token ${tokenAddress} to Telegram`
        );
      }
    }
  } catch (error) {
    console.error(
      `[${getTimeStamp()}] Error checking token ${tokenAddress}:`,
      error
    );
  }
}
