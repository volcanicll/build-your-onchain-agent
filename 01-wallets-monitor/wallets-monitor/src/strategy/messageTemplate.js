...
function formatSmartMoney(analysis) {
  ...
  for (const [address, data] of Object.entries(analysis)) {
    details += `\u{25AB}<a href="https://solscan.io/account/${address}">${
      data.walletName
    }</a> bought ${formatNumber(data.totalBuyCost)} at MC ${formatNumber(
      data.averageMarketCap
    )}(${data.buyTime}), Holds: ${data.holdsPercentage}\n`;
  }
  return details.trim();
}

// Creates a formatted message with token information and smart money analysis
export function createMsg(tokenInfo, analysis, analyzeTokenInfo) {
...
<a href="https://dexscreener.com/solana/${
    tokenInfo.address
  }">DexScreener</a> | <a href="https://gmgn.ai/sol/token/${
    tokenInfo.address
  }">GMGN</a>${
    tokenInfo.website ? ` | <a href="${tokenInfo.website}">Website</a>` : ""
  }${tokenInfo.twitter ? ` | <a href="${tokenInfo.twitter}">Twitter</a>` : ""}
`.trim();
}
