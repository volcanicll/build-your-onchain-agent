...
export async function searchTwitter(query, searchType = "Top") {
  ...
  const response = await axiosClient
    .get(
      `https://twitter-api45.p.rapidapi.com/search.php?query=${query}&search_type=${searchType}`,
      {
        headers: {
          "x-rapidapi-key": RAPID_API_KEY,
          "x-rapidapi-host": TWITTER_API_HOST,
  ...
  return response.data.timeline.map((tweet) => ({
    ...
    created_at:
      new Date(tweet.created_at).toLocaleString("en-US", { hour12: false }) +
      " UTC", // Published time, format: 2025/2/25 14:41:14 UTC
...
export async function getUserTimeline(screenname) {
  ...
  if (response.data.pinned) {
    result.tweets.push({
      ...
      created_at:
        new Date(response.data.pinned.created_at).toLocaleString("en-US", {
          hour12: false,
        }) + " UTC",
