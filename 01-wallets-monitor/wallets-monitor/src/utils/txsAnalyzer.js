...
async function getTokenTxs(tokenAddress) {
  try {
    ...
    const { data: txs } = await supabase
      ...
      .or(
        `token_in_address.eq.${tokenAddress},token_out_address.eq.${tokenAddress}`
      )
...
async function analyzeTxs(accountTxs, tokenAddress) {
  ...
  for (const [account, txs] of Object.entries(accountTxs)) {
    ...
    const remainingAmount = BigNumber.maximum(
      0,
      totalBuyAmount.minus(totalSellAmount)
    );
    const holdsPercentage = remainingAmount
      .dividedBy(totalBuyAmount)
      .multipliedBy(100);
  ...
  const resultWithNames = Object.entries(result).reduce(
    (acc, [address, data]) => {
      acc[address] = {
        ...data,
...
export async function analyzeTokenTxs(tokenAddress) {
  try {
    ...
    console.error(
      `Error analyzing transactions for token ${tokenAddress}:`,
      error
    );
    throw error;
  }
